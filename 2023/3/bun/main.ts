...
const sumOfPartNumbers = engineSchematic
  .split("\n")
  .map((row) => row.split(""));

const eachMatrix = (
  matrix: string[][],
  eachFn: (char: string, coord: [number, number], matrix: string[][]) => void
) => {
...
const cb = (char: string, coord: [number, number], matrix: string[][]) => {
  console.log("char", char);
  console.log("coord", coord);
};

eachMatrix(sumOfPartNumbers, cb);
