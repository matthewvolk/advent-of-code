...
const part1 = () => {
  ...
  for (const game of games) {
    ...
    rounds.forEach((round) => {
      const numCubesByColor = round.split(", ");

      numCubesByColor.forEach((n) => {
        const [num, color] = n.split(" ");
        if (color === "red" && parseInt(num) > 12) {
          roundValid = false;
        }

        if (color === "green" && parseInt(num) > 13) {
          roundValid = false;
        }

        if (color === "blue" && parseInt(num) > 14) {
          roundValid = false;
        }
      });
    });
...
const part2 = () => {
  ...
  for (const game of games) {
    ...
    rounds.forEach((round) => {
      ...
      numCubesByColor.forEach((n) => {
        const [num, color] = n.split(" ");
        if (color === "red" && parseInt(num) > minRed) {
          minRed = parseInt(num);
        }

        if (color === "green" && parseInt(num) > minGreen) {
          minGreen = parseInt(num);
        }

        if (color === "blue" && parseInt(num) > minBlue) {
          minBlue = parseInt(num);
        }
      });
    });
